...
describe("Payments", function () {
  ...
  beforeEach(async function(){
    ...
    await payments.waitForDeployment(); //ждем завершения транзакции развертывания
  ...
  it("should be possible to send funds", async function () { //проверка на то что контракт развернут
    ...
    await expect(() => tx) // анонимная функция
      .to.changeEtherBalance(
        [acc2.target, payments.target], //ethers v5 --> ethers v6
        [-sum, sum]
      );
    ...
    const newPayment = await payments.getPayment(acc2.target, 0); //вызов, не транзакция
    console.log(newPayment);
    expect(newPayment.message).to.eq(msg);
    expect(newPayment.amount).to.eq(sum);
    expect(newPayment.from).to.eq(acc2.target);
  });
  ...
  it("should emit PaymentSent event when paying", async function () {
    ...
    console.log(`Отправка ${sum} wei с сообщением: "${msgText}" от ${acc2.target}`);
    ...
    await expect(tx)//проверяем, что транзакция выполнена без ошибок
      ...
      .withArgs(acc2.target, sum, msgText);
  });
